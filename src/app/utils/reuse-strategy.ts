...
export class SimpleReuseStrategy implements RouteReuseStrategy {
  ...
  retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle {
    return !route.data.key
      ? null
      : SimpleReuseStrategy.handlers[route.data.key];
  }
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    const ret =
      !!route.data.key && !!SimpleReuseStrategy.handlers[route.data.key];

    if (ret) {
      this.runHook(
        '_onReuseInit',
        SimpleReuseStrategy.handlers[route.data.key].componentRef
      );
    }
    return ret;
  }
